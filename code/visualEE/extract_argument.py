...
for i_f, g, p, p_r in zip(img_files, gold, pred, pred_roles):
    ...
    if ans == g and ans != 'None':
        print(i_f)
        print(ans)
        print(list(set(res_role_for_plot)))
        print('=====')

        for l, r in list(set(res_role_for_plot)):
            image_path = '../data/m2e2_rawdata/m2e2_rawdata/image/image/' + i_f + '.jpg'
            output_path = 'result/' + i_f + '_' + l + '_' + r + '.jpg'
            print('python Transformer_MM/visualization_function.py %s %s %s %s %s' % (image_path, l, output_path, ans, r))
            os.system('python Transformer_MM/visualization_function.py %s %s %s %s %s' % (image_path, l, output_path, ans, r))

            # plot(image_path, l, output_path, event=ans, noun=r)
